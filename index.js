...
import { URLs } from './user-data/urls.js';
  ...
  const { semanticsearch, devopsautomation, llmrag } =
    projects;
  ...
  const mediumRSSFeedURL = 'https://medium.com/@mrpksjr/feed';
  
  async function getMediumPostsAsJSON() {
    try {
      const response = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(mediumRSSFeedURL)}&api_key=${apiKey}`);
      const data = await response.json();
      const posts = data.items; // Array of posts in JSON format
      
      // Iterate through items
      posts.forEach(post => {
      // Check if description contains a figure tag
      const figureMatch = post.description.toString().match(/<img[^>]+src="([^">]+)"/);
      
      
      if (figureMatch && figureMatch.length > 1) {
        // Assign the thumbnail value from figure tag to the thumbnail key
        post.thumbnail = figureMatch[1];
      }
    });
      // Process and display the posts as needed
      populateBlogs(posts, "blogs");
    } catch (error) {
  ...
  getMediumPostsAsJSON();
  fetchGitConnectedData(gitConnected);
  
  populateProjects(semanticsearch, "semantic-search");
  populateProjects(devopsautomation, "devops-automation");
  populateProjects(llmrag, "llm-rag");
